...
object FileResolver {
  ...
  def hasFile(path: String): Boolean = {
    if(config.env() == "production") hasResourceFile(path)
    else hasLocalFile(path)
  }

  def hasDirectory(path: String) = {
    if (config.env() == "production") false
    else hasLocalDirectory(path)
  }
