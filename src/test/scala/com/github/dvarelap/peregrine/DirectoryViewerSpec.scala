...
class DirectoryViewerSpec extends ShouldSpec {
  ...
  "DirectoryViewer" should "not crash under normal conditions" in {
    try {
      FileResolver.getLocalDirectory("public/components").get()
    } catch {
  ...
  it should "crash under abnormal conditions" in {
    try {
      FileResolver.getLocalDirectory("/public/components/imaginary/folder").get
  ...
  it should "create a correct ListingView" in {
    ...
    listView.files.toSet should
      be(Set(DirectoryViewer.FileView("/components/bootstrap.css", "bootstrap.css"), DirectoryViewer.FileView("/components/test_file.txt",
        "test_file.txt")))
